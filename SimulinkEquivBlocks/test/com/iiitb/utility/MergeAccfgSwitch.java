...
public class MergeAccfgSwitch {
	...
	public static Accfg mergeTest(ArrayList<Block> blockList) {
		...
		merged = new Accfg();
		 ...
		 Iterator<String> iterKey =  FetchInputFromLineSwitch.adjacencyList.keySet().iterator();
		 ...
		 while(iterKey.hasNext())
		 {
			 ...
			 Iterator<DestNode> tempIter = FetchInputFromLineSwitch.adjacencyList.get(key).iterator();
		   ...
		   * As per our model it should contain - Constant , Constant1,Constant2,Switch
		   * 
		   * sortedList should contain same blocks as blockList but topographically sorted
		   * 
		   * The list size should be 4 in both the cases
		   * 
		   * 
		   * */
		
		  assertEquals(4, blockList.size());
		  Block[] test = {new Constant("Constant"),new Constant("Constant1"),new Constant("Constant2"),new Switch("Switch")};
		  assertArrayEquals(test, blockList.toArray());

		  
		  assertEquals(4, sortedList.size());
		  String[] testSort = {"Constant2","Constant1","Constant","Switch"};
		   ...
		   *  Constant=1
		   *  Constant2=1
		   *  Switch=(Switch=(Switch=(Constant2 > 0) OR (Constant2 = 0))?Constant:Constant1)
		   * 
		...
		while(test7Iter.hasNext())
		{
			...
			assertTrue(tempFp, tempFp.equalsIgnoreCase("Switch=((Constant2 > 0)?Constant:Constant1)") || tempFp.equalsIgnoreCase("Constant1=8") || tempFp.equalsIgnoreCase("Constant=1")||tempFp.equalsIgnoreCase("Constant2=1"));
	...
	public static Accfg findInputOutputVariableTest(Accfg accfg,
		 ...
		 *  Constant2 ==> input - ****  			   , output - Constant2
		 *  Switch 	  ==> input - Constant , Constant1 & Constant2, output - Switch 
		 *  
		...
		assertEquals("Switch", ((Variable)output.get(0)).getName());
