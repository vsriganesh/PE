...
public class FetchInputFromLineRelationalOperator {
	...
	public void parseLineTest(ArrayList<Block> blockList,
		 ...
		 * 
		 * As per our model this basically contains 'Constant,Constant1,Relational+"\n"+"Operator'
		 * 
		 ...
		 * TEST:2 
		 * 
		 * If the source node is 'Constant' or 'Constant1' , as per the model destination node should be RelationalOperator
		 * 
		 * 
		 * */
		...
		{
			assertEquals("RelationalOperator",nodeIni.getName().replaceAll("\n", ""));
		}
		 ...
		 *  Check for any key the corresponding value list has Relational+"\n"+"Operator
		 * 
		 * 
		 * */
		...
		if (sourceNode != "") {
			...
			while (blockListIter.hasNext()) {
				 ...
				 * </Line>
				 * 
				 * is parsed , input for 'RelationalOperator' block should be set as 'Constant' */
				
				if(((Variable)blockObj.getOutput()).getName().equalsIgnoreCase("Relational"+"\n"+"Operator"))
				{
		...
		while (blockListIter.hasNext()) {
			...
			if (blockObj.isInputSetFlag()
				 ...
				 * 
				 * 
				 * 	Check if the input for "Relational"+"\n"+"Operator" block is 'Constant' and 'Constant1'
				 * 
				 *  Check if expression created for "Relational"+"\n"+"Operator" block is -> Relational"+"\n"+"Operator=(Relational"+"\n"+"Operator=(Constant < Constant1) OR (Constant = Constant1))
				...
				if(((Variable)blockObj.getOutput()).getName().equalsIgnoreCase("Relational"+"\n"+"Operator"))
				{
					...
					assertEquals(blockObj.expression().toString(),"Relational"+"\n"+"Operator=((Constant < Constant1) OR (Constant = Constant1))");
