...
public class FetchInputFromLineDivide {
	...
	public void parseLineTest(ArrayList<Block> blockList,
		 ...
		 * 
		 * As per our model this basically contains 'Constant,Constant1,Divide'
		 * 
		 ...
		 * TEST:2 
		 * 
		 * If the source node is 'Constant' or 'Constant1' , as per the model destination node should be Divide
		 * 
		 * 
		 * */
		...
		{
			assertEquals("Divide",nodeIni.getName());
		}
		 ...
		 *  Check for any key the corresponding value list has 'Divide'
		 * 
		 * 
		 * */
		...
		if (sourceNode != "") {
			...
			assertEquals(adjacencyList.get(sourceNode).get(0).getName(),"Divide");
			...
			while (blockListIter.hasNext()) {
				 ...
				 * </Line>
				 * 
				 * is parsed , input for 'Divide' block should be set as 'Constant' */
				
				if(((Variable)blockObj.getOutput()).getName().equalsIgnoreCase("Divide"))
				{
		...
		while (blockListIter.hasNext()) {
			...
			if (blockObj.isInputSetFlag()
				 ...
				 * 
				 * 
				 * 	Check if the input for 'Divide' block is 'Constant' and 'Constant1'
				 * 
				 *  Check if expression created for 'Divide' block is -> Divide = Constant/Constant1
				...
				if(((Variable)blockObj.getOutput()).getName().equalsIgnoreCase("Divide"))
				{
					...
					assertEquals(blockObj.expression().toString(),"Divide=(Constant / Constant1)");
