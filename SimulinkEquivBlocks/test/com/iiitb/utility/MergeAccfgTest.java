...
public class MergeAccfgTest {
	...
	public static Accfg mergeTest(ArrayList<Block> blockList) {
		...
		merged = new Accfg();
		 ...
		 Iterator<String> iterKey =  FetchInputFromLineTest1.adjacencyList.keySet().iterator();
		 ...
		 while(iterKey.hasNext())
		 {
			 ...
			 Iterator<DestNode> tempIter = FetchInputFromLineTest1.adjacencyList.get(key).iterator();
		  ...
		  Block[] test = {new Constant("Constant"),new Constant("Constant1"),new Sum("Sum")};
		  assertArrayEquals(test, blockList.toArray());

		  
		  assertEquals(3, sortedList.size());
		  String[] testSort = {"Constant1","Constant","Sum"};
		...
		while(test7Iter.hasNext())
		{
			...
			assertTrue(tempFp, tempFp.equalsIgnoreCase("Sum=(Constant + Constant1)") || tempFp.equalsIgnoreCase("Constant1=8") || tempFp.equalsIgnoreCase("Constant=1"));
	...
	public static Accfg findInputOutputVariableTest(Accfg accfg,
		 ...
		 *  Sum 	  ==> input - Constant & Constant1 , output - Sum 
		 *  
		...
		assertEquals("Sum", ((Variable)output.get(0)).getName());
