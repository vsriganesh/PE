...
public class MergeAccfgTestDelay {
	...
	public static Accfg mergeTest(ArrayList<Block> blockList) {
		...
		merged = new Accfg();
		 ...
		 Iterator<String> iterKey =  FetchInputFromLineDelay.adjacencyList.keySet().iterator();
		 ...
		 while(iterKey.hasNext())
		 {
			 ...
			 Iterator<DestNode> tempIter = FetchInputFromLineDelay.adjacencyList.get(key).iterator();
		   ...
		   * As per our model it should contain - Constant , Constant1,Sum
		   * 
		  ...
		  Block[] test = {new Constant("Constant"),new Sum("Delay"),new Delay("Sum")};
		  assertArrayEquals(test, blockList.toArray());
		 
		  
		  assertEquals(3, sortedList.size());
		  String[] testSort = {"Constant","Sum","Delay"};
		   ...
		   *  
		   *  As per our model , initList and delayList will have values as per the delay block
		   *  
		...
		while(test7Iter.hasNext())
		{
			...
			assertTrue(tempFp, tempFp.equalsIgnoreCase("Sum=(Delay + Constant)") || tempFp.equalsIgnoreCase("Constant=2"));
		...
		// Delay will have an initialization
		assertEquals(1, initList.size());
		
		
		merged.setDelay(delayList);
		assertEquals(5, delayList.size());
	...
	public static Accfg findInputOutputVariableTest(Accfg accfg,
		 ...
		 *  Delay ==> input - Sum 			   , output - Delay
		 *  Sum 	  ==> input - Constant & Delay , output - Sum 
		 *  
