...
public class MergeAccfgMinMax {
	...
	public static Accfg mergeTest(ArrayList<Block> blockList) {
		...
		merged = new Accfg();
		 ...
		 Iterator<String> iterKey =  FetchInputFromLineMinMax.adjacencyList.keySet().iterator();
		 ...
		 while(iterKey.hasNext())
		 {
			 ...
			 Iterator<DestNode> tempIter = FetchInputFromLineMinMax.adjacencyList.get(key).iterator();
		   ...
		   * As per our model it should contain - Constant ,MinMax
		   * 
		   * sortedList should contain same blocks as blockList but topographically sorted
		   * 
		   * The list size should be 2 in both the cases
		   * 
		   * 
		   * */
		
		  assertEquals(2, blockList.size());
		  Block[] test = {new Constant("Constant"),new MinMax("MinMax")};
		  assertArrayEquals(test, blockList.toArray());

		  
		  assertEquals(2, sortedList.size());
		  String[] testSort = {"Constant","MinMax"};
		...
		while(test7Iter.hasNext())
		{
			...
			assertTrue(tempFp, tempFp.equalsIgnoreCase("MinMax=min([Constant])") || tempFp.equalsIgnoreCase("Constant=1"));
	...
	public static Accfg findInputOutputVariableTest(Accfg accfg,
		 ...
		 *  
		 *  MinMax	  ==> input - Constant, output - MinMax 
		 *  
		...
		assertEquals("MinMax", ((Variable)output.get(0)).getName());
