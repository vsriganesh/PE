...
public class FetchInputFromLineSwitch {
	...
	public void parseLineTest(ArrayList<Block> blockList,
		 ...
		 * 
		 * As per our model this basically contains 'Constant,Constant1,Constant2,Switch'
		 * 
		 ...
		 * TEST:2 
		 * 
		 * If the source node is 'Constant', 'Constant1' or 'Constant2' , as per the model destination node should be Switch
		 * 
		 * 
		 * */

		if(sourceNode.equalsIgnoreCase("Constant") || sourceNode.equalsIgnoreCase("Constant1")||sourceNode.equalsIgnoreCase("Constant2"))
		{
			assertEquals("Switch",nodeIni.getName());
		}
		 ...
		 *  Check for any key the corresponding value list has Switch
		 * 
		 * 
		 * */
		...
		if (sourceNode != "") {
			...
			while (blockListIter.hasNext()) {
				 ...
				 * </Line>
				 * 
				 * is parsed , input for 'Switch' block should be set as 'Constant' */
				
				if(((Variable)blockObj.getOutput()).getName().equalsIgnoreCase("Switch"))
				{
		...
		while (blockListIter.hasNext()) {
			...
			if (blockObj.isInputSetFlag()
				 ...
				 * 
				 * 
				 * 	Check if the input for "Switch" block is 'Constant', 'Constant1' and 'Constant2'
				 * 
				 *  Check if expression created for "Switch" block is -> Switch=(Switch=(Switch=(Constant2 > 0) OR (Constant2 = 0))?Constant:Constant1)
				...
				if(((Variable)blockObj.getOutput()).getName().equalsIgnoreCase("Switch"))
				{
					String[] testInput = {"Constant","Constant1","Constant2"};
					...
					assertEquals(blockObj.getInput().toArray()[2].toString(),testInput[2]);
					//System.out.println("Test "+blockObj.getClass());
					/*
					 * Switch Expression cannot be tested here as we are creating variables inside
					 * Expression method of Switch block.Same named variable cannot exist in the same program scope
					 */
					//assertEquals(blockObj.expression().toString(),"Switch=(Switch=(Switch=(Constant2 > 0) OR (Constant2 = 0))?Constant:Constant1)");
