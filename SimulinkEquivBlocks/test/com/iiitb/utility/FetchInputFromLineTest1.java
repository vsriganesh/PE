...
public class FetchInputFromLineTest1 {
	...
	public void parseLineTest(ArrayList<Block> blockList,
		 ...
		 * TEST:2 
		 * 
		 * If the source node is 'Constant' or 'Constant1' , as per the model destination node should be sum
		 * 
		 * 
		 * */
		 ...
		 *  Check for any key the corresponding value list has 'Sum'
		 * 
		 * 
		 * */
		...
		while (blockListIter.hasNext()) {
			...
			if (blockObj.isInputSetFlag()
				 ...
				 * 
				 * 
				 * 	Check if the input for 'Sum' block is 'Constant' and 'Constant1'
				 * 
				 *  Check if expression created for 'Sum' block is -> Sum = Constant+Constant1
				...
				if(((Variable)blockObj.getOutput()).getName().equalsIgnoreCase("Sum"))
				{
					...
					assertEquals(blockObj.expression().toString(),"Sum=(Constant + Constant1)");
