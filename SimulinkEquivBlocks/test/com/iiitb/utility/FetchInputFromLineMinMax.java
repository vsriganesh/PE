...
public class FetchInputFromLineMinMax {
	...
	public void parseLineTest(ArrayList<Block> blockList,
		 ...
		 * 
		 * As per our model this basically contains 'Constant,MinMax'
		 * 
		 ...
		 * TEST:2 
		 * 
		 * If the source node is 'Constant' , as per the model destination node should be MinMax
		 * 
		 * 
		 * */

		if(sourceNode.equalsIgnoreCase("Constant"))
		{
			assertEquals("MinMax",nodeIni.getName());
		}
		 ...
		 *  Check for any key the corresponding value list has 'MinMax'
		 * 
		 * 
		 * */
		...
		if (sourceNode != "") {
			...
			assertEquals(adjacencyList.get(sourceNode).get(0).getName(),"MinMax");
			...
			while (blockListIter.hasNext()) {
				 ...
				 * </Line>
				 * 
				 * is parsed , input for 'MinMax' block should be set as 'Constant' */
				
				if(((Variable)blockObj.getOutput()).getName().equalsIgnoreCase("MinMax"))
				{
		...
		while (blockListIter.hasNext()) {
			...
			if (blockObj.isInputSetFlag()
				 ...
				 * 
				 * 
				 * 	Check if the input for 'MinMax' block is 'Constant'
				 * 
				 *  Check if expression created for 'MinMax' block is -> MinMax = Min([Constant])
				...
				System.out.println("blockObect"+blockObj.expression());
				
				if(((Variable)blockObj.getOutput()).getName().equalsIgnoreCase("MinMax"))
				{
					String[] testInput = {"Constant"};
...
//					assertEquals(blockObj.getInput().toArray()[1].toString(),testInput[1]);
					assertEquals(blockObj.expression().toString(),"MinMax=Min([Constant])");
